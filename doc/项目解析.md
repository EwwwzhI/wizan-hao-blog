# `wutong-yu` 项目解析（读到依赖`@astrojs/check`）
...
- 当前真正参与页面渲染的主要是 `home`、`blogs`、`projects`、`insights`
...
| `data.ts`     | 博客与 Insights 的过滤、排序、按年份分组、阅读时长辅助 |
| `misc.ts`     | 面板淡入淡出、滚动锁定          |
| `toc.ts`      | TOC 树生成              |
...
- 生成正文预览，并在超过 200 字时显示“展开全文 / 收起内容”
...
- 提供页面级 frontmatter，例如 `title`、`subtitle`、`description`、`ogImage`
...
### 16.7 `src/pages/app.webmanifest.js`
...
- 静态页面共 13 个
- `/`、`/blogs/`、`/blogs/*`、`/projects/`、`/insights/`、`/404.html`、`/app.webmanifest` 都已正常生成
